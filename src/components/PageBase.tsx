...
const redirectProps = (destination: string, isPermanent: boolean) => ({
    ...
    redirect: {
        ...
        // If isPermanent is valid, use this, else default to true
        permanent: (isPermanent !== undefined && isPermanent !== null ? isPermanent : true),
    },
});
...
export const fetchPageProps = async (
    ...
    if (isMediaContent(content)) {
        return redirectProps(getMediaUrl(content.mediaUrl, isDraft), false);
    }
    ...
    if (isNotFound(content)) {
        ...
        if (sanitizedPath !== xpPath) {
            return redirectProps(stripXpPathPrefix(sanitizedPath), true);
        }
