...
export const scrapeProductPageForProductDetails = (
    ...
    const sectionComponents =
        productPage?.pageProps?.content?.page?.regions['pageContent']
            ?.components;
    ...
    const productDetails = sectionComponents.reduce(
        (collection: PartComponentProps[], section: LayoutCommonProps) => {
            const components =
                section.regions?.content?.components ||
                ([] as PartComponentProps[]);
            const newComponents = [];

            components.forEach((component: PartComponentProps) => {
                if (
                    !component ||
                    component.descriptor !== PartType.ProductDetails
                ) {
                    return;
                }
